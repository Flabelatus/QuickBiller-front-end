            ...
            <div className="App-header row justify-content-center" style={{minWidth: 400}}>
                <header className='d-flex justify-content-center align-items-center pb-1 mb-1 mt-4'>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {jwtToken === "" && <Link to="/register"><p className='btn btn-submit-dark-small ms-3 mt-2'><FontAwesomeIcon icon={faUser} className="me-2" />Sign Up</p></Link>}
                        {jwtToken !== "" && <p><Link to="/user_company" className='mt-2 btn btn-light ms-5 px-4' style={{ color: 'black', fontSize: 20, width: 'fit-content', border: '1px solid #ccc', borderRadius: 25, fontWeight: 350 }}>My Company Info</Link></p>}
                        {jwtToken !== "" && <p><Link to="/history" className='mt-2 btn btn-light ms-5 px-4' style={{ color: 'black', fontSize: 20, width: 'fit-content', border: '1px solid #ccc', borderRadius: 25, fontWeight: 350 }}>Overview</Link></p>}
                        {jwtToken !== "" && <p><Link onClick={handleLogout} className='mt-2 btn btn-light- btn ms-5' style={{ color: '#888', fontSize: 20, }}><FontAwesomeIcon icon={faSignOut} className="me-2" />Logout</Link></p>}
                    </div>
                </header>
            </div>
        </>
