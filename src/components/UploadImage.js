...
export const UploadImage = ({ setImageData, setImageName }) => {
    ...
    const handleUpload = () => {
        ...
        if (selectedFile) {
            ...
            fetch(`${process.env.REACT_APP_BACKEND}/logged_in/upload?uID=${userID}`, {
                method: "POST",
                ...
                fetch(`${process.env.REACT_APP_BACKEND}/logged_in/image/${data.data.filename}`, requestOptionsImage)
                    .then((r) => r.blob())
                    .then((image) => {
